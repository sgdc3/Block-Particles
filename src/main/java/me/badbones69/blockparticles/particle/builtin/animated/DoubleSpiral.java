...
public class DoubleSpiral extends AbstractParticle {

    public DoubleSpiral() {
        super(new Vector(.5, .7, .5), 2, location -> new AnimationTickHandler(16) {
            ...
            public void frame(int currentFrame, final List<Player> receivers) {
                ...
                ParticleEffect.FIREWORKS_SPARK.send(receivers, current.getKey(), 0, 0, 0, 0, 1);
                ParticleEffect.FIREWORKS_SPARK.send(receivers, current.getValue(), 0, 0, 0, 0, 1);
            }
        });
    }
}
